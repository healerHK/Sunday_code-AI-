...
def speak(audio):
    with open('D:\SUNDAY\\texttt.txt', mode='w',encoding='UTF-8') as f:
        f.write(audio)
    print('Healer:'+audio)
    friday.say(audio)
    friday.runAndWait()   
...
def run():
    ...
    #program 7:------------------------------------------Control led--------------------------------------------------------
            elif "bật đèn chính" in query or "bật đèn" in query:
                    Arduino_Serial.write('9'.encode())
                    speak("đèn đã được bật")
            elif "đèn đỏ" in query or "bật đèn màu đỏ" in query or "bật đèn đỏ" in query:
                den_do()
            elif "đèn xanh lá" in query or "bật đèn màu xanh lá" in query or "bật đèn xanh lá" in query:
                den_xanh_la()
            elif ("đèn xanh nước biển" or "bật đèn màu xanh nước biển" or "bật đèn xanh nước biển") in query:
                den_xanh_nuoc_bien()
            elif ("đèn vàng" or "bật đèn vàng" or "bật đèn màu vàng") in query:
                den_vang()
            elif ("đèn tím" or "bật đèn màu tím" or "bật đèn tím") in query:
                den_tim()
            elif ("đèn trắng" or "bật đèn trắng" or "bật đèn màu trắng") in query:
                den_trang()
            elif "đèn xanh lam" in query or "bật đèn xanh lam" in query or 'bật đèn màu xanh lam' in query:
                den_xanh_lam()
            elif "đèn 7 màu" in query  or "bật đèn 7 màu" in query:
                den_7_mau()
            elif ("tắt đèn" or "turn off") in query:
                Arduino_Serial.write('0'.encode())
                speak("tất cả đèn đã tắt")
    ...
    #Program 13:--------------------------------------Alarm_clock---------------------------------------------------------
                ...
                set_alarm.set_time()
    #program 14:--------------------------------------MY_DOOR--------------------------------------------------------------
            elif 'mở cửa' in query:
                Arduino_Serial.write('10'.encode())
                speak("cửa đã được mở")
            elif 'đóng cửa' in query:
                Arduino_Serial.write('11'.encode())
                speak("cửa đã đóng")
    #program 15:-------------------------------------Sunday_boy---------------------------------------------------------------
            elif 'bật robot quét nhà' in query:
                SB.on_robot()
                speak("robot quét nhà đã bật")
            elif 'tắt robot quét nhà' in query:
                SB.off_robot()
                speak("robot quét nhà đã tắt")
    ...
    #Program 18:------------------------------------Sunday_SOS--------------------------------------------------------------
            elif 'cấp cứu' in query or 'cứu tôi' in query or 'liên lạc với người thân' in query:
                Arduino_Serial.write('12'.encode())
                time.sleep(3)
                speak("Tôi đã liên lạc với người thân, Sếp hãy cố gắng lên")
                time.sleep(2)
                Arduino_Serial.write('0'.encode())
    #Program 19:-----------------------------------Yamail_Face_detect-----------------------------------------------------
                ...
                with open("D:\SUNDAY\Face_detect\Face_detect.txt", mode='w', encoding='UTF-8') as ff:
                    ff.write("có")  
                speak('chế độ vắng nhà đã được bật, tôi sẽ gửi gmail cho sếp khi có người tới nhà')
